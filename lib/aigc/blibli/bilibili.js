...
export async function isToolInstalled (toolName) {
  ...
  try {
    ...
    logger.info(`Tool detected: ${toolName}`)
    toolCheckCache[toolName] = true
    return true
  } catch (error) {
    logger.warn(`Tool not detected: ${toolName}. Will use fallback.`) 
...
export async function downloadWithAria2c (videoUrl, audioUrl, videoPath, audioPath, headers) {
  logger.info('Attempting parallel download with aria2c.')
...
export async function downloadWithNativeFetch (videoUrl, audioUrl, videoPath, audioPath, headers) {
  logger.info('Using native fetch downloader as fallback.')
  ...
  logger.info('Native downloads complete.')
}
...
export async function getBilibili (bvid) {
  try {
    ...
    try {
      ...
      logger.warn(`Failed to fetch stream size for BVID ${bvid}:`, err)
    }
    ...
    logger.error(`Error fetching data for BVID ${bvid}:`, err)
    return null
  }
}
