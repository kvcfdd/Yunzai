...
const AUDIO_DIR = "data/aigc/voice"
...
async function downloadAudio(url) {
  ...
  try { await fs.promises.access(filePath); return filePath } catch { /* miss */ }

  const res = await fetch(url)
  if (!res.ok) throw new Error(`音频下载失败 [${res.status}]`)

  const buffer = Buffer.from(await res.arrayBuffer())
  await fs.promises.mkdir(AUDIO_DIR, { recursive: true }).catch(() => { })
  await fs.promises.writeFile(filePath, buffer)
  log.debug(`音频已缓存: ${filePath}`)
  return filePath
}
