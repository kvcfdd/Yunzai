...
export async function isToolInstalled(toolName) {
  ...
  try {
    ...
    logger.debug(`检测到外部工具: ${toolName}`)
    toolCheckCache[toolName] = true
    return true
  } catch {
    logger.warn(`未找到外部工具: ${toolName}`)
...
export async function mergeVideoAndAudio(videoPath, audioPath, outputPath) {
  ...
  try {
    ...
    logger.error(`视频合并失败: ${err.stderr || err.message}`)
    if (err.code === "ENOENT") {
...
export async function getBilibili(bvid) {
  try {
    ...
    try {
      ...
      logger.debug(`获取视频大小失败: ${err.message}`)
    }
    ...
    logger.error(`获取B站视频信息失败: ${err.message}`)
    return null
  }
}
