  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      if (!reasoning_content && content.includes("<think>")) {
        ...
        if (matches.length) {
          reasoning_content = matches.map(m => m[1].trim()).filter(Boolean).join("\n")
          content = content.replace(/<think>[\s\S]*?<\/think>/g, "").trim()
        } else {
          // 有 <think> 但无闭合对，清理残留标签
...
async function geminiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      let reasoning_content = thoughtParts.join("") || undefined

      // 兜底：如果模型未使用原生 thought 标记，而是把思维链以 <think> 标签写在文本中
      if (!reasoning_content && content.includes("<think>")) {
        const thinkRegex = /<think>([\s\S]*?)<\/think>/g
        const matches = [...content.matchAll(thinkRegex)]
        if (matches.length) {
