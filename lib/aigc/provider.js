...
/** 下载图片并存入本地缓存，非标准 MIME 类型统一存为 jpg */
    ...
    if (msg.role === "user" && msg.images?.length) {
      ...
      for (let i = 0; i < limit; i++) {
        try {
          ...
          logger.debug(`图片下载失败: ${err.message}`)
        }
      }
...
async function openaiChat(messages, options = {}) {
  ...
  if (cfg.aigc?.thinking) body.reasoning_effort = "medium"
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      logger.debug(`OpenAI API 调用完成 ${model} ${Date.now() - t0}ms`)
      ...
      if (attempt < rotated.length - 1) {
        logger.warn(`OpenAI API 尝试 ${attempt + 1}/${rotated.length} 失败: ${err.message}`)
      }
    }
  }
...
async function geminiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      logger.debug(`Gemini API 调用完成 ${model} ${Date.now() - t0}ms`)

      if (!candidate?.content) {
        if (candidate?.finishReason) {
          logger.warn(`Gemini 内容安全拦截: ${candidate.finishReason}`)
        }
      ...
      lastError = err
      if (attempt < rotated.length - 1) {
        logger.warn(`Gemini API 尝试 ${attempt + 1}/${rotated.length} 失败: ${err.message}`)
      }
    }
  }
