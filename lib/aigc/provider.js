...
import { getLLMDispatcher } from "./proxy.js"
    ...
    if (msg.role === "user" && msg.images?.length) {
      ...
      for (let i = 0; i < limit; i++) {
        try {
          ...
          logger.warn(`图片下载失败: ${err.message}`)
        }
      }
...
async function openaiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      const oaiTokens = usage ? `  prompt=${usage.prompt_tokens}  completion=${usage.completion_tokens}  total=${usage.total_tokens}` : ""
      logger.info(`API  openai  ${model}${oaiTokens}  ${Date.now() - t0}ms`)
      ...
      if (attempt < rotated.length - 1) {
        logger.warn(`重试  openai  ${attempt + 1}/${rotated.length}  ${err.message}`)
      }
    }
  }
...
async function geminiChat(messages, options = {}) {
  ...
  for (let attempt = 0; attempt < rotated.length; attempt++) {
    ...
    try {
      ...
      const gemTokens = usage ? `  prompt=${usage.promptTokenCount}  completion=${usage.candidatesTokenCount}  total=${usage.totalTokenCount}` : ""
      logger.info(`API  gemini  ${model}${gemTokens}  ${Date.now() - t0}ms`)

      if (!candidate?.content) {
        if (candidate?.finishReason) {
          logger.warn(`Gemini拦截  ${candidate.finishReason}`)
        }
      ...
      lastError = err
      if (attempt < rotated.length - 1) {
        logger.warn(`重试  gemini  ${attempt + 1}/${rotated.length}  ${err.message}`)
      }
    }
  }
