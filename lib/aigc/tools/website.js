...
tools.register({
  name: "fetch_website",
  description: "访问指定网页，提取正文内容并转为 Markdown 格式。用于获取网页详细信息辅助回答。可通过 useProxy 参数控制是否走本地代理（需管理员已配置代理地址）。",
  parameters: {
    type: "object",
    properties: {
      url: {
      ...
      useProxy: {
        type: "boolean",
        description: "是否走本地代理，默认 false。部分网站需要代理才能访问",
      },
    },
    required: ["url"],
  },
  execute: async (args) => {
    ...
    if (!urlRegex.test(url)) return "无效的 URL 格式"
    ...
    try {
      ...
      if (!response || response.status() >= 400) {
        return `访问失败: HTTP ${response?.status() || "未知"}`
      }
      ...
      await page.evaluate(() => {
        ...
        document.querySelectorAll("img, picture").forEach(img => {
          if (img.alt?.trim()) {
            img.parentNode.replaceChild(document.createTextNode(` [图:${img.alt}] `), img)
          } else {
            img.remove()
          }
        })
      ...
      if (article?.content) {
        pageTitle = article.title || "无标题"
        finalContent = turndown.turndown(article.content)
      } else {
        pageTitle = doc.window.document.title || "无标题"
      ...
      if (msg.includes("Timeout")) msg = "导航超时"
