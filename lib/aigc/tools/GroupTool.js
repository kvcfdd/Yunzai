...
tools.register({
  name: "group_admin",
  description: "Group admin operations: kick, kick_batch, ban (mute), unban (unmute), set_card (change member nickname), set_admin, set_title (special title), set_name (group name), set_whole_ban (mute all), unset_whole_ban (unmute all), send_notice, quit (leave group). For poke/like use the interact tool. Verify permissions before executing. Only use when user explicitly requests.",
  parameters: {
    type: "object",
    properties: {
      action: {
