...
tools.register({
  name: "invoke_command",
  description: "Execute a bot command on behalf of the user. Takes a full command string including prefix (e.g. '#绑定123456'). The command output is returned as text or forwarded to the chat.",
  parameters: {
    type: "object",
    properties: {
      command: {
  ...
  execute: async (args, ctx) => {
    ...
    const synthEvent = { ...origEvent, _synthetic: true }
    synthEvent.msg = commandStr
    ...
    try {
      ...
      log.error(`invoke_command 失败: ${err.message}`)
    ...
    // 文本结果直传 LLM，图片/视频/音乐等转发给用户并返回摘要
    const llmParts = []
    for (let i = 0; i < results.length; i++) {
      ...
      if (r.type === "text") {
        ...
        try {
          ...
          log.warn(`invoke_command 转发失败: ${err.message}`)
        }
