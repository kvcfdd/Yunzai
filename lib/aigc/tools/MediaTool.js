...
tools.register({
  name: "send_media",
  description: "Send a music card (Netease) or video (Bilibili). Use search(type='music'|'video') FIRST to get the ID. NOT for images — use search(type='image') + send_image for pictures.",
  parameters: {
    type: "object",
    properties: {
  ...
  execute: async (args, ctx) => {
    ...
    if (type === "music") {
      try {
        ...
        logger.error(`发送音乐失败: ${err.message}`)
    ...
    if (type === "video") {
      ...
      try {
        ...
        logger.debug(`视频合并完成: ${id}`)

        await e.reply({ type: "video", data: { file: outputPath } })
        return `Video sent`
      } catch (err) {
        logger.error(`发送视频失败: ${err.message}`)
