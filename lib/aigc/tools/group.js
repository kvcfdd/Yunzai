...
tools.register({
  name: "group_admin",
  description: "Group admin operations: kick, kick_batch, ban (mute), unban (unmute), set_card (change member nickname), set_admin, set_title (special title), set_name (group name), set_whole_ban (mute all), unset_whole_ban (unmute all), poke, send_notice, quit (leave group). Verify permissions before executing.",
  parameters: {
    type: "object",
    properties: {
      action: {
      ...
      user_id: {
        type: "number",
        description: "Target user QQ. Required for: kick, ban, unban, set_card, set_admin, set_title, poke",
      },
      user_ids: {
        type: "array",
        items: { type: "number" },
      ...
      duration: {
        type: "number",
        description: "Mute duration in seconds. Default: 300",
      },
      new_card: {
        type: "string",
        description: "New group card (nickname). Required for: set_card",
      },
      title: {
        type: "string",
        description: "Special title text. Required for: set_title. Pass empty string to remove title",
      },
      enable: {
        type: "boolean",
        description: "true to set as admin, false to remove admin. Required for: set_admin",
      },
      name: {
        type: "string",
        description: "New group name. Required for: set_name",
      },
      content: {
        type: "string",
        description: "Announcement content. Required for: send_notice",
      },
      image: {
        type: "string",
        description: "Announcement image URL, optional. For: send_notice",
      },
      reject_add_request: {
        type: "boolean",
        description: "Reject rejoin requests after kick. Default: false. For: kick, kick_batch",
      },
    },
  ...
  execute: async (args, ctx) => {
    ...
    if (action !== "poke") {
      try {
        ...
        if (!botMember) {
          return `Cannot get bot member info in group ${group_id}`
        }
        if (OWNER_ACTIONS.includes(action)) {
          if (botMember.role !== "owner") {
            return `Bot must be group owner to execute '${action}'`
          }
        } else {
          if (botMember.role !== "admin" && botMember.role !== "owner") {
            return `Bot is not an admin in group ${group_id}`
          }
        }
      } catch {
    ...
    try {
      switch (action) {
        ...
        case "poke": {
          if (!user_id) return "poke requires 'user_id'"
          await group.pokeMember(user_id)
          return `Poked member ${user_id}`
        }

        /* ---- Identity/card operations ---- */
        case "set_card": {
