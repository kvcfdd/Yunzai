...
tools.register({
  name: "render_html",
  description: "Render Markdown or HTML content as an image and send to the user. Use for tables, charts, formatted text, code highlighting, etc.",
  parameters: {
    type: "object",
    properties: {
      content: {
        type: "string",
        description: "Markdown or HTML content to render. Defaults to Markdown parsing.",
      },
      width: {
        type: "integer",
        description: "Image width in pixels. Default: 780",
      },
    },
    required: ["content"],
  },
  execute: async (args, ctx) => {
    ...
    try {
      ...
      return "[Image replied directly to user]"
    } catch (err) {
