...
tools.register({
  name: "browse",
  description: "Fetch a web page, extract main content and convert to Markdown. Use after search(type='web') for detailed content.",
  parameters: {
    type: "object",
    properties: {
  ...
  execute: async (args) => {
    ...
    if (!(await isHostnameSafe(parsed.hostname))) {
      logger.warn(`拦截内网地址: ${parsed.hostname}`)
