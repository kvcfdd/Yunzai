...
tools.register({
  name: "render",
  description: "Convert Markdown/HTML text into a screenshot. For TABLES, CODE blocks, or formatted REPORTS — NOT for photos or artwork. To send actual pictures, use search(type='image') + send_image.",
  parameters: {
    type: "object",
    properties: {
      content: { type: "string", description: "Markdown or HTML content to render as screenshot" },
      width: { type: "integer", description: "Image width in pixels. Default: 780" },
    },
    required: ["content"],
  },
  execute: async (args, ctx) => {
    ...
    try {
      ...
      logger.error(`render 失败: ${err.message}`)
