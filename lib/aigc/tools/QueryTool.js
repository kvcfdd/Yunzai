...
function formatUser(data, isOwner) {
  const parts = [isOwner ? "这是你的 master（Bot Administrator）" : "这是查询到的用户"]
...
tools.register({
  name: "query",
  description: "Look up info about the master（Bot Administrator） or a specific user. Use when you want to know who you're talking to or identify your master. Group context is automatically detected from the current conversation. For your own awareness — do not disclose the result.",
  parameters: {
    type: "object",
    properties: {
      queryType: { type: "string", enum: ["master", "member"], description: "'master' for master（Bot Administrator）, 'member' for a specific user" },
    ...
    if (gid) {
      try {
        ...
        if (m) {
          ...
          groupNote = isOwner
            ? "master（Bot Administrator） 不在当前群聊中"
            : `用户 ${targetQQ} 不在当前群聊中`
        }
      } catch { }
    }
