...
class McpClient {
  ...
  async _initialize() {
    ...
    logger.info(`[aigc] mcp init  server=${this.name}  version=${result?.protocolVersion}  session=${this.sessionId || "none"}`)
  ...
  async _request(method, params, { isInitialize = false, retried = false } = {}) {
    ...
    if (res.status === 404 && this.sessionId && !isInitialize && !retried) {
      logger.warn(`[aigc] mcp session expired  server=${this.name}  reconnecting`)
      await this._initialize()
  ...
  async _notify(method, params) {
    try {
      ...
      if (!res.ok && res.status !== 202) {
        logger.warn(`[aigc] mcp notify  server=${this.name}  method=${method}  status=${res.status}`)
      }
    } catch (err) {
      logger.warn(`[aigc] mcp notify  server=${this.name}  method=${method}  err=${err.message}`)
    }
  }
